...
function Templater( options ) {
	...
	self.compile = function( options ) {
		var deferred = Q.defer();
		try {
			var compiled = dust.compile( options.content, self.settings.namespace + options.identifier );
	...
	self.render = function( options ) {
		...
		options.data.templateNamespace = self.settings.namespace;

		// Render the template
		dust.render( self.settings.namespace + options.template, options.data, function( err, out ) {
			if( err ) {
				deferred.reject( err );
			} else {
