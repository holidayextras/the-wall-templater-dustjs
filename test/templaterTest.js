...
var filesRequired = {
	test: __dirname + '/fixtures/wallTemplater/templates/test.tpl',
	partial: __dirname + '/fixtures/wallTemplater/templates/partial.tpl',
	syntaxError: __dirname + '/fixtures/wallTemplater/templates/syntaxError.tpl'
};

// FooData is special.. is a json object so load it as such.
testFiles.fooData = require( __dirname + '/fixtures/foo.json' );
...
describe( 'Wall Templater', function() {
	...
	it( 'should fail on invalid key in the schema', function( done ) {
		...
		} ).should.throw( 'ValidationError: newKey is not allowed' );
		done();
	} );
	...
	describe( '#compile', function() {
		...
		it( 'should fulfill a promise when successful', function() {
			return templater.compile( {
				identifier: __dirname + '/fixtures/wallTemplater/templates/test.tpl',
		...
		it( 'should reject a promise when failure occurs', function() {
			return templater.compile( {
				identifier: __dirname + '/fixtures/wallTemplater/templates/noFileHere.tpl',
		...
		it( 'should reject a promise when syntax error is found', function() {
			return templater.compile( {
				identifier: __dirname + '/fixtures/wallTemplater/templates/syntaxError.tpl',
	...
	describe( '#render', function() {
		...
		it( 'should populate the template with data', function() {
			...
			// Load a partial then render template
			return templater.compile( {
				identifier: __dirname + '/fixtures/wallTemplater/templates/partial.tpl',
				content: testFiles['partial']
			} ).then( function() {
				return templater.render( {
					template: 'test',
